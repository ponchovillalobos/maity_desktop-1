    ...
    pub async fn switch_audio_device(
        ...
        info!(
            ...
            match device_type {
                ...
                DeviceMonitorType::SystemAudio => "system audio",
            },
            device_name
        );
        ...
        if let Some(device) = device {
            ...
            match device_type {
                DeviceMonitorType::Microphone => {
                    ...
                    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
                    self.stream_manager
                        .start_streams(Some(device_arc.clone()), system_device.clone(), None)
                        .await?;
                    ...
                    self.restart_device_monitor(Some(device_arc), system_device)
                        .await;
                ...
                DeviceMonitorType::SystemAudio => {
                    ...
                    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
                    self.stream_manager
                        .start_streams(microphone_device.clone(), Some(device_arc.clone()), None)
                        .await?;
                    ...
                    // Update device monitor to watch the new device
                    self.restart_device_monitor(microphone_device, Some(device_arc))
                        .await;
    ...
    async fn restart_device_monitor(
        ...
        if let Some(ref mut monitor) = self.device_monitor {
            ...
            if let Err(e) = monitor.start_monitoring(microphone, system_audio) {
                warn!(
                    "Failed to restart device monitoring after device switch: {}",
                    e
                );
            } else {
