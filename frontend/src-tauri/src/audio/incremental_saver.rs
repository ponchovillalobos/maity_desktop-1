...
impl IncrementalAudioSaver {
    ...
    pub fn new(meeting_folder: PathBuf, sample_rate: u32, channels: u16) -> Result<Self> {
        ...
        info!(
            "IncrementalAudioSaver: {} channels, {}Hz, 30s checkpoints",
    ...
    async fn merge_checkpoints(&self, output: &PathBuf) -> Result<()> {
        ...
        command.args(&[
            ...
            "-i",
            list_file.to_str().ok_or_else(|| {
                anyhow::anyhow!("List file path contains invalid UTF-8: {:?}", list_file)
            })?,
            "-c",
            "copy", // Copy codec - no re-encoding!
            "-y",   // Overwrite output file
            output.to_str().ok_or_else(|| {
                anyhow::anyhow!("Output path contains invalid UTF-8: {:?}", output)
            })?,
        ]);
...
pub async fn recover_audio_from_checkpoints(
    ...
    command.args(&[
        ...
        "0",
        "-i",
        concat_file_path.to_str().ok_or_else(|| {
            format!(
                "Concat file path contains invalid UTF-8: {:?}",
                concat_file_path
            )
        })?,
        "-c",
